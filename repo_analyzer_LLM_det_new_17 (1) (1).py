...
def _sampled_real_loc(prov, info, code_paths, sample_cap=300):
    """Real per-file line counts via the API, on a bounded sample,
...
class GitLabProvider(HttpBase):
    ...
    def commit_dates(self, info):
        ...
        pages = (int(total) if total and total.isdigit()
...
def _authenticated_clone_url(web_url, kind, token):
    """Embed the access token into the HTTPS clone URL so `git clone` works
    for private repos without an interactive login prompt. Public repos
    clone fine even without this (token is simply omitted)."""
    if not token or not web_url.startswith("https://"):
        return web_url
...
def measure_remote_pytest_coverage(web_url, kind, token, primary_language="",
    ...
    tmp_dir = tempfile.mkdtemp(prefix="repo_cov_")
    try:
        clone_url = _authenticated_clone_url(web_url, kind, token)
        try:
            r = subprocess.run(
            ...
            return ""
        if r.returncode != 0:
            # Mask the token if it leaked into stderr (e.g. in the URL on
            # some git versions) before printing.
...
CSV_COLUMNS = [
    "Project/Group name", "Provider", "Established year",
...
def main():
    ...
    ap.add_argument("--skip-local-coverage", action="store_true",
                    help="Local mode only: skip actually running the repo's "
                         "pytest suite to measure real coverage %% (this is "
                         "the only step that executes the target repo's "
                         "code — skip it if that's undesirable/unsafe, or "
                         "if the repo has no Python test suite)")
    ap.add_argument("--skip-remote-coverage", action="store_true",
                    help="GitLab/GitHub mode: when the API/pipeline doesn't "
                         ...
                         "the repo and running its pytest suite to measure "
                         "real coverage. This clones the repo and executes "
                         "its code (Python/pytest only) — skip it if that's "
                         "undesirable/unsafe, slow, or the repo isn't "
                         "Python/pytest-based")
    ...
    for p in projects:
        try:
            r = analyze_project(prov, p, args.max_mrs, args.sample_mrs,
                                ...
                                skip_local_coverage=args.skip_local_coverage,
                                skip_remote_coverage=args.skip_remote_coverage)
            if r:
                results.append(r)
